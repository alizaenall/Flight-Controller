...
// Interval between points for 100 sample per second.
#define LOG_INTERVAL_USEC 10000  // 10ms
// Size to log 100 byte lines at 100 Hz for more than ten minutes.
#define LOG_FILE_SIZE 100 * 100 * 600  // 6,000,000 bytes for 10 minute
// Space to hold more than 100 ms of data for 100 byte lines at 100 sps.
#define RING_BUF_CAPACITY 512
#define LOG_FILENAME "QuadLog2.csv"
...
// PWM Motor
  ...
  rb.begin(&file);
  Serial.println("SD card siap untuk logging.");
...
void logData() {
  ...
  uint32_t logTime = micros();
    ...
    if (micros() < logTime) {}
      ...
      rb.print(spareMicros); rb.write(','); 
      rb.print(RateRoll); rb.write(',');
      rb.print(RatePitch); rb.write(',');
      rb.print(RateYaw); rb.write(',');
      rb.print(Roll); rb.write(',');
      rb.print(Pitch); rb.write(',');
      rb.print(Yaw); rb.write(',');
      rb.println(distance);
